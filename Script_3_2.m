...
rastr = 0;  % O sistema possui sistema de rastreamento em dois eixos
    ...
    crit_max = 4;  % Valor máximo para o critério utilizado
...
if ~rastr && ~conc % Parâmetros de entrada para otimização da inclinação e orientação
    beta_max = 90;
    beta_min = 0;
    beta_inc = 5;
    gamma_max = 0;
...
elseif ~rastr && conc
    ...
    % Inicialização dos valores de ângulo horário de limite inferior, 
    % superior e valor médio da hora
    omega_1 = floor(-max(omega_s)/15)*15 : 15 : floor(max(omega_s)/15)*15;
    omega_2 =  ceil(-max(omega_s)/15)*15 : 15 : ceil(max(omega_s)/15)*15;
    omega = (omega_1 + omega_2)/2;
...
if ~conc && ~rastr
    ...
    % Obtenção do ponto com  máxima geração de energia anual e marcação no
    % gráfico
...
%     
%     % Nova figura com os valores em porcentagem do valor de geração máxima
%     % de energia
%     figure; hold on; grid on;
...
%     xlabel('Ângulo de azimute');
%     ylabel('Ângulo de inclinação');
%     titulo = strcat('Porcentagem em relação a produção ótima de energia- ',cidades{cid},'  (', num2str(abs(lat)), 'ºS)[kWh/m²]');
%     title(titulo);
%     view([90 90]);
...
elseif ~conc && rastr
    ...
    disp('Escolha ainda não implementada.')
end