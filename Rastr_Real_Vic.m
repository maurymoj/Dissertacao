% Simulação de rastreamento para a cidade de Viçosa - MG com dados medidos
% a partir de estação meteorológica com intervalo entre medições de 15
% minutos.
...
[delta,omega_s] = Geom_Solar_Dia(lat, n_trat);   % delta = Declinação solar
...
for j = 1:length(mods_Rastr)
    ...
    for crit = crit_min:crit_inc:crit_max
        ...
        g_rastr(j,i) = sum(sum(G_t_r.*(G_t_r(:,:)>0 | isreal(G_t_r(:,:))),2))/sum(sum(G_t_perez.*(G_t_perez(:,:)>0 | isreal(G_t_perez(:,:))),2));
...
for j = 1:length(mods_Rastr)
    plot(n_reord_m(j,:), real(g_rastr(j,:)));
end
