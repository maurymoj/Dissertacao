function G_t = Rad_Inc_Dia_Claro(G, G_b, G_d, G_o, R_b, beta_sup, rho_g, mod, varargin)
...
if strcmpi(mod, 'HDKR')
    ...
    f = sqrt(G_b./G);
    A_i = G_b./G_o;
    G_t = max( ( G_b + G_d.*A_i ).*R_b ...
               + G_d.*( 1 - A_i ).*((1 + cosd(beta_sup) )/2).* ...
                                  (1 + f.*sind(beta_sup/2).^3) ...
               + G*rho_g.*(1-cosd(beta_sup))/2,0 );
elseif strcmpi(mod, 'Perez')
    ...
    G_bn = G_b./cosd(theta_z);         %CONFIRMAR USO
    
    % Parâmetro de claridade
    eps_c = ((G_d + G_bn)./G_d + 5.535e-6*theta_z.^6)./(1+ 5.535e-6*theta_z.^3);

    % Massa de ar
    m = real((theta_z < 70).* 1./cosd(theta_z)+...
    ...
    G_on = G_sc*(1.000110 + 0.034221*cosd(B) + 0.001280*sind(B)+...
    ...
    G_t = G_b.*R_b + G_d.*(1 - F_1).*(1+cosd(beta_sup))/2 + G_d.*F_1.*a./b...
          + G_d.*F_2.*sind(beta_sup) + G*rho_g.*(1-cosd(beta_sup))/2;
