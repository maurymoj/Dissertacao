...
inc_fix = 0; % Estudo de sistema fixo
...
elseif inc_var
    beta_min = 0;
    beta_inc_1 = 5;% Incremento no primeiro loop de procura da radiação máxima
    beta_inc_2 = 1;% Incremento no segundo loop de procura da radiação máxima
    beta_max = 90;
    n_inc = [1 2 4 12]; % Número de inclinações usadas ao longo do ano
    legend_var = {'Fixed horizontal','Yearly optimal tilt','Semi-yearly optimal tilt','Seasonal optimal tilt','Monthly optimal tilt'};
end
...
if inc_fix
    ...
    colormap('hot');
    c = colorbar;
    xlabel('Azimuth angle (°)','FontName','Times New Roman','FontSize',12);
    ylabel('Tilt angle (°)','FontName','Times New Roman','FontSize',12);
    xlabel(c,'MJ/m²/year','FontName','Times New Roman','FontSize',12);
    %axes.
    ...
    eval(sprintf('print -dtiff -r600 cidade%dFix',cid))
...
elseif inc_var
    ...
    legenda = legend_var{1};
    for i=1:length(n_inc)
        plot(H_inc(:,i+1));
    end
  
    legend(legend_var,'Location', 'SouthEast');
	xlabel('Months','FontName','Times New Roman','FontSize',12);
    ylabel('Monthly mean daily radiation (MJ/m²/day)','FontName','Times New Roman','FontSize',12);
    fig = gcf;
    fig.PaperPositionMode = 'auto';
    eval(sprintf('print -dtiff -r600 cidade%dVar',cid));
elseif rastr
    if dia_claro == 1
