...
cidades = {'Manaus - AM', 'Garanhuns – PE', 'Brasília – DF', 'Belo Horizonte – MG', 'Campo Grande – MS', 'Joaçaba – SC','São Gabriel – RS'};
...
cid = 6; % Definição da cidade onde sera feita a simulacao
...
for beta_sup = 0:5:90
    ...
    for gamma_sup = 180:-5:90
        ...
        Z_dados(n_dados, m_dados) = sum(sum(I_t_perez,2));
    end
%plot(gamma_sup,sum(sum(I_t_perez,2)),'x');
    m_dados = 0;
end

surf(0:5:90,180:-5:90,Z_dados);
colormap('hot');
colorbar;
xlabel('Ângulo de inclinação');
ylabel('Ângulo de azimute');
titulo = strcat('')
view([90 90]);
